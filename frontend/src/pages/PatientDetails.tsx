...
const PatientDetails: React.FC = () => {
  ...
  // Centralized clinical metrics calculation
  const metrics = useMemo(() => {
    if (!patient) return null;
    return calculateClinicalMetrics(patient);
  }, [patient]);

  const projectedCDR = metrics?.futureCDR || 0;
  const currentCDRValue = metrics?.currentCDR || 0;
  ...
  const mlInsights = useMemo(() => {
    ...
    return {
      ...
      declineRate: metrics.declineRate > 0 ? `${metrics.declineRate.toFixed(1)} pts / yr` : "Stable"
    };
  }, [metrics]);
  ...
  const patientStatus = useMemo(() => {
    ...
    if (metrics.statusLabel === 'Critical Alert') icon = AlertCircle;
    else if (metrics.statusLabel === 'Monitor') icon = Clock;
  ...
  const chartData = useMemo(() => {
    ...
    return sortedTests.map((t, i) => {
      // Calculate a local CDR for each historical point for the graph
      let pointCdr = 0.0;
      if (t.totalScore < 15) pointCdr = 2.0;
      else if (t.totalScore < 20) pointCdr = 1.0;
      else if (t.totalScore < 26) pointCdr = 0.5;
              ...
              <div className="relative bg-[#FAF9F8] border border-[#EDEBE9] rounded-sm p-4 h-[400px] flex items-end justify-between overflow-visible">
                <svg className="absolute inset-0 w-full h-full p-12 overflow-visible" viewBox={`0 0 ${CHART_WIDTH} ${CHART_HEIGHT}`} preserveAspectRatio="none">
                  ...
                  {chartData.length > 1 && (
                    <>
                      ...
                      <path
                        ...
                        strokeLinecap="round"
                      />
                      {/* CDR Score Line (Blue Dotted) */}
                      <path
                        d={`M ${chartData.map((d, i) => `${PADDING_X + (i / (chartData.length - 1)) * (CHART_WIDTH - 2 * PADDING_X)},${CHART_HEIGHT - PADDING_Y - (d.cdr * 6 / 30) * (CHART_HEIGHT - 2 * PADDING_Y)}`).join(' L ')}`}
                        fill="none"
                        stroke="#0078D4"
                        strokeWidth="2"
                        strokeDasharray="6,4"
                      />
                      {/* Projection Line (Red Dotted) - Lengthened and vertical scale reduced to decrease steepness */}
                      <line
                        x1={CHART_WIDTH - PADDING_X}
                        y1={CHART_HEIGHT - PADDING_Y - (chartData[chartData.length - 1].cdr * 6 / 30) * (CHART_HEIGHT - 2 * PADDING_Y)}
                        x2={CHART_WIDTH + 35}
                        y2={CHART_HEIGHT - PADDING_Y - (projectedCDR * 6 / 30) * (CHART_HEIGHT - 2 * PADDING_Y)}
                        stroke="#D83B01"
                        strokeWidth="3.5"
                        strokeDasharray="8,4"
                      />
                  ...
                  {chartData.length > 0 && chartData.map((d, i) => {
                    const xPos = chartData.length > 1
                      ? PADDING_X + (i / (chartData.length - 1)) * (CHART_WIDTH - 2 * PADDING_X)
                      : CHART_WIDTH / 2;
                    const yPosMoca = CHART_HEIGHT - PADDING_Y - (d.moca / 30) * (CHART_HEIGHT - 2 * PADDING_Y);
                    const yPosCdr = CHART_HEIGHT - PADDING_Y - (d.cdr * 6 / 30) * (CHART_HEIGHT - 2 * PADDING_Y);
                    ...
                    return (
                      <g key={i}>
                        ...
                        {/* CDR Data Points (Blue Dots) */}
                        <circle cx={xPos} cy={yPosCdr} r="4" fill="#0078D4" className="pointer-events-none" />
                ...
                {hoveredPoint !== null && (
                  <div
                    ...
                    style={{
                      left: `calc(48px + (${(chartData.length > 1 ? (PADDING_X + (hoveredPoint / (chartData.length - 1)) * (CHART_WIDTH - 2 * PADDING_X)) : CHART_WIDTH / 2) / CHART_WIDTH} * (100% - 96px)))`,
            ...
            <div className="bg-white border border-gray-300 p-8 flex flex-col items-center">
                ...
                <svg viewBox={`0 0 ${CHART_WIDTH} ${CHART_HEIGHT}`} className="w-full h-full overflow-visible">
                  ...
                  {chartData.length > 1 && (
                    <>
                      ...
                      <path d={`M ${chartData.map((d, i) => `${PADDING_X + (i / (chartData.length - 1)) * (CHART_WIDTH - 2 * PADDING_X)},${CHART_HEIGHT - PADDING_Y - (d.cdr * 6 / 30) * (CHART_HEIGHT - 2 * PADDING_Y)}`).join(' L ')}`} fill="none" stroke="#0078D4" strokeWidth="2" strokeDasharray="6,4" />
                      {/* Projection Line for Print (Red Dotted) - Lengthened and vertical scale reduced */}
                      <line
                        x1={CHART_WIDTH - PADDING_X}
                        y1={CHART_HEIGHT - PADDING_Y - (chartData[chartData.length - 1].cdr * 6 / 30) * (CHART_HEIGHT - 2 * PADDING_Y)}
                        x2={CHART_WIDTH + 160}
                  ...
                  {chartData.map((d, i) => {
                    ...
                    return (
                      <g key={i}>
                        {/* MoCa Point */}
                        <circle cx={xPos} cy={CHART_HEIGHT - PADDING_Y - (d.moca / 30) * (CHART_HEIGHT - 2 * PADDING_Y)} r="5" fill="black" />
                        {/* CDR Point (Blue Dot) */}
                        <circle cx={xPos} cy={CHART_HEIGHT - PADDING_Y - (d.cdr * 6 / 30) * (CHART_HEIGHT - 2 * PADDING_Y)} r="4" fill="#0078D4" />
                        <text x={xPos} y={CHART_HEIGHT} className="text-[10px] font-bold fill-gray-600" textAnchor="middle">{d.date}</text>
                      </g>
                    );
                  })}
                </svg>
              ...
              <div className="mt-12 grid grid-cols-2 gap-10 w-full px-10">
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 text-xs"><div className="w-3 h-3 bg-[#0078D4] border-dashed border"></div> <span>Historical CDR (Blue Dotted)</span></div>
                  <div className="flex items-center gap-2 text-xs"><div className="w-3 h-3 bg-[#D83B01] border-dashed border"></div> <span>1Y CDR Projection (Red Dotted)</span></div>
                </div>
