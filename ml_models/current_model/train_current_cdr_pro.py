...
def train_moca_only_model():
    # 1. Configuration
    output_dir = 'current_model'
    data_path = 'data_current.csv'
    ...
    print(f"Loaded {len(df)} rows for current CDR prediction.")
    ...
    print("\n" + "="*30)
    print("   MoCA-LONGITUDINAL CURRENT MODEL")
    print("="*30)
    ...
    plt.title('MoCA-Longitudinal: Current CDR Confusion Matrix')
    ...
    plt.title('Feature Importance (MoCA-Longitudinal Current)')
    ...
    joblib.dump(model_data, os.path.join(output_dir, 'moca_current_model.joblib'))
    print(f"\nAll files saved to '{output_dir}/' folder.")

if __name__ == "__main__":
    train_moca_only_model()
